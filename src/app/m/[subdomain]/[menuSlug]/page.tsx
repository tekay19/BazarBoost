...
export default async function PublicMenuPage({ params }: { params: { subdomain: string; menuSlug: string } }) {
  const data = await loadPublicMenu(params.subdomain, params.menuSlug);
  if (!data) notFound();
  ...
  prisma.menuView
    ...
    .then(() => prisma.business.update({ where: { id: data.businessId }, data: { lastActivityAt: new Date() } }))
    .catch(() => {});
